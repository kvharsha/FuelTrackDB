...
<p align="center">
  A modern, map-first fuel-station locator and management platform for Bengaluru.<br/>
  <b>React + Vite + TypeScript + MUI + Leaflet × Django + DRF × SQL × Docker + PWA</b>
</p>
...
**FuelTrackDB** delivers a Google-Maps-like experience for fuel stations:

- 🔑 **User/Admin authentication**
- 🗺️ **Full-screen map UI** with smooth transitions
- 🧭 **Directions** using grid A* pathfinding + **Haversine distance/ETA**
- ⛽ **Fuel filters**: CNG, Petrol, Diesel, EV (AC/DC), LPG + availability & price sorting
- ⭐ **Favorites**, 📍 **Recents**, 📝 **Reviews**
- 📴 **Offline/PWA** (Dexie + service worker)
- ⚙️ **Admin Portal**: stations, fuels, users, reviews, analytics

> Data importers fetch **Bengaluru** stations from Maruti (CNG), Statiq (EV), and OpenStreetMap/Overpass (Petrol/Diesel).
...
- [Data Import (Bengaluru)](#-data-import-bengaluru)
- [Testing](#-testing)
...
### 👤 User Experience
- Glass-morphism **Auth** (Login/Signup) with username/email availability checks
- **Map-first** interface post-login; immediate geolocation prompt
- Floating UI:
  - **Welcome toast** (greets first name; gentle “breathe” hover)
  - **Top-right avatar** → Preferences & **Day/Night** toggle (half-moon)
  - **Bottom-right**: Locate-me, Zoom ±, **Wi-Fi strength** (latency buckets)
  - **Center search**: station or place (fuzzy)
  - **Left ribbon** → expandable **Side Panel**: Favorites, Recents, fuel chips, reviews, recent locations, offline packages
  - **Layer picker** (bottom-left): Geographical / Satellite / Terrain
- **Markers** with fade-in & **price color** gradient (green=cheapest nearby → red)
- **Routing**: grid A* polyline + **Haversine** distance & **ETA**
- **Onboarding modal**, **compass**, **battery** indicator (if supported)
- **Map position persistence** (center/zoom)

### 🛠️ Admin Portal
- CRUD: stations (with fuels/price/availability), fuel types, users (block/unblock), reviews (moderate)
- **MUI DataGrid** inline edit, pagination, search/filter
- **Charts** (ratings distribution, average price)
- **CSV exports**, **toast** notifications

### ⚙️ Platform & Quality
- DRF Token Auth, permissioned routes (admin-only writes)
- **Rate-limited** login (5/min/IP)
- `/api/health/` endpoint for container monitoring
- PWA via `vite-plugin-pwa` (assets, data caching)
- Production Dockerfiles (Gunicorn + Nginx), secure headers & CORS
- Unit tests for auth/nearby/favorites/reviews/admin
...
| Layer       | Technologies                                                                                                                      |
|-------------|-----------------------------------------------------------------------------------------------------------------------------------|
| Frontend    | React 18 · Vite · TypeScript · **MUI** · Emotion · **React-Leaflet** · **Redux Toolkit** · **Dexie** · Axios · Router · Motion · PWA |
| Backend     | **Django 5** · **Django REST Framework** · authtoken · CORS headers · MySQL                                                       |
| Data import | Maruti Suzuki CNG (scraped) · Statiq EV (scraped) · **OpenStreetMap Overpass** (amenity=fuel)                                    |
...
```mermaid
flowchart LR
    A[Frontend (React/Vite/TS)] -->|Axios| B[DRF API (Django)]
    B -->|ORM + Stored Proc| C[(MySQL)]
